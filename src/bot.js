...
const { handleTextResponse, handleCallbackQuerys } = require('./lib/top');
...
const influencerQuestions = [
    ...
    {
        text: 'Where does your business receives payments?',
...
bot.hears('RENEW PLAN', async (ctx) => {
    try {
            const userId = ctx.from.id;
            const userData = await bot.User.get(`${userId}`);
...
bot.command('clearsubscriber', async (ctx) => {
    const userId = ctx.message.text.split(' ')[1]; // Extract user ID from the command argument
    ...
    try {
        let topTenUsers = await readJsonSafely(topTenPath);

        if (topTenUsers[userId]) {
            delete topTenUsers[userId];
            await fs.writeJson(topTenPath, topTenUsers);

            await ctx.reply(`User ${userId} has been successfully removed from the Top 10 Influencers package.`);
            await ctx.telegram.sendMessage(userId, 'Your subscription to the Top 10 Influencers package has been cleared by an admin.');
        } else {
            await ctx.reply(`User ${userId} is not subscribed to the Top 10 Influencers package.`);
        }
    } catch (error) {
        console.error('Error clearing subscriber:', error);
        await ctx.reply('An error occurred while trying to clear the subscriber.');
    }
});
...
bot.hears('CONTACT SUPPORT', async (ctx) => {
            ...
            await ctx.reply(
                'If you have any questions, kindly reach out to our human support.',
                {
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: 'CUSTOMER SUPPORT', url: url }]
                        ]
                    }
                }
            );
...
const topTenPath = './topTenUsers.json';
const maxTopTenUsers = 10;
const weeklySubscriptionDuration = 7 * 24 * 60 * 60 * 1000; // 7 days in milliseconds

bot.hears('TOP 10 TRENDING INFLUENCERS', async (ctx) => {
    try {
        const userId = ctx.from.id;
        const userData = await bot.User.get(`${userId}`);

        if (!userData || userData.status !== 'Approved' || userData.role !== 'influencer') {
            await ctx.telegram.sendChatAction(ctx.chat.id, 'typing');
            await ctx.reply('You need to be an approved influencer to access this plan.');
            return;
        }

        let topTenUsers = await readJsonSafely(topTenPath);
        const userAlreadySubscribed = topTenUsers[userId];
        const now = Date.now();

        if (userAlreadySubscribed && now - userAlreadySubscribed.startTime < weeklySubscriptionDuration) {
            await ctx.reply('You are already subscribed to this plan for the current week.');
            return;
        }

        if (Object.keys(topTenUsers).length >= maxTopTenUsers) {
            await ctx.reply('The Top 10 Influencers package is fully booked for this week.');
            return;
        }

    const imagePath = path.join(__dirname, './img/top.jpg'); // Replace with actual path to the image
    const caption = `
<b>Top 10 Trending Influencers of the Week 🎉

Only 10 spots available weekly!

Select a Subscription plan ♻️

1 WEEK ⭐️⭐️⭐️ (29% Off)
<s>₦70,000</s>➔ ₦50,000 / Week</b>`;
    
    const paymentMessage = await ctx.replyWithPhoto({ source: createReadStream(imagePath) }, {
        caption: caption,
        reply_markup: {
            inline_keyboard: [
                [{ text: 'Subscribe', callback_data: 'subscribe_top_ten' }]
            ]
        },
        parse_mode: 'HTML'
    });
        ...
        ctx.session.messageIds = ctx.session.messageIds || [];
        ctx.session.messageIds.push(paymentMessage.message_id);

    } catch (error) {
        console.error('Error in TOP 10 TRENDING INFLUENCERS:', error);
    }
});

bot.action('subscribe_top_ten', async (ctx) => {
    try {
        await ctx.telegram.sendChatAction(ctx.chat.id, 'typing');
        const amount = 50000; // ₦50,000
        const transaction = await initializePayment(ctx, amount);

        if (transaction) {
            const paymentMessage = await ctx.reply("Click the button below to complete your payment:", {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: 'Pay Now', web_app: { url: transaction.authorization_url } }],
                        [{ text: 'I\'ve made the payment', callback_data: `Auth_${transaction.reference}` }]
                    ]
                }
            });
            ctx.session.messageIds.push(paymentMessage.message_id);
        ...
        console.error('Error in subscribe_top_ten:', error);
...
bot.action(/^Auth_/, async (ctx) => {
    try {
        await ctx.telegram.sendChatAction(ctx.chat.id, 'typing');
        const reference = ctx.callbackQuery.data.split('_')[1];
        ...
        if (transaction.status === 'success') {
            ...
            let topTenUsers = await readJsonSafely(topTenPath);
            topTenUsers[userId] = { startTime, plan: 'top_ten_influencers' };
            await fs.writeJson(topTenPath, topTenUsers);

            let successfulPayments = await readJsonSafely(successfulPaymentsPath);
            successfulPayments[userId] = transaction.reference;
            await fs.writeJson(successfulPaymentsPath, successfulPayments);

            await ctx.reply('Your payment was successful! You have been added to the Top 10 Influencers for this week.');

            await startSubscriptionTiming(ctx, userId, startTime);
...
async function startSubscriptionTiming(ctx, userId, startTime) {
    const timeRemaining = startTime + weeklySubscriptionDuration - Date.now();
    
    if (timeRemaining <= 0) {
        await expireSub(ctx, userId);
        return;
    }
...
async function expireSub(ctx, userId) {
    let topTenUsers = await readJsonSafely(topTenPath);

    if (topTenUsers[userId]) {
        delete topTenUsers[userId];
        await fs.writeJson(topTenPath, topTenUsers);
        await ctx.telegram.sendMessage(userId, 'Your subscription to the Top 10 Influencers plan has ended.');
    }
}

async function initializePayment(ctx, amount) {
    try {
        const userData = await bot.User.get(`${ctx.from.id}`);
        ...
        const requestBody = {
            email: email,
            amount: amount * 100,
            metadata: { chatId: ctx.from.id, plan: 'top_ten_influencers' },
        ...
        await ctx.reply('Error initializing payment. Please try again.');
        return null;
    }
}
...
bot.on('callback_query', async (ctx) => {
    ...
    if (data.startsWith('Buy_')) {
        await handleCallbackQuerys(ctx, bot);
        return;
    }
...
async function askNextQuestion(ctx) {
    ...
    async function simulateTyping(ctx, duration = 1000) {
        await ctx.telegram.sendChatAction(ctx.from.id, 'typing');
        await new Promise(resolve => setTimeout(resolve, duration));
    }

    await simulateTyping(ctx);
...
async function handleIncorrectMediaType(ctx, question) {
    ...
    const repeatQuestionMessage = await ctx.reply(question.text, options);
...
async function forwardAnswersToAdmin(ctx) {
    ...
    let message = `User <b>${escapeHtml(ctx.from.first_name)}</b> (ID: <code>${ctx.from.id}</code>) answered the following:\n\n`;
    for (const [key, value] of Object.entries(answers)) {
        const safeKey = escapeHtml(key);
        const safeValue = Array.isArray(value) ? value.map(escapeHtml).join(', ') : escapeHtml(value);
        message += `<b>${safeKey}</b>: ${safeValue}\n\n`;
    }
    ...
    const sendPhotoUsingFileId = async (fileId, caption) => {
        try {
            ...
            message += `<b>${escapeHtml(caption)}</b>: Failed to send photo. Error: ${escapeHtml(err.message)}\n`;
        }
    };
    ...
    const sendVideoUsingFileId = async (fileId, caption) => {
        try {
            ...
            message += `<b>${escapeHtml(caption)}</b>: Failed to send video. Error: ${escapeHtml(err.message)}\n`;
        }
    };
...
async function handleCallbackQuery(ctx) {
    ...
    const influencerSubRoles = ['mega_influencer', 'standard_influencer', 'micro_influencer'];

    if (influencerSubRoles.includes(role)) {
  
    } else if (role === 'designer') {

    } else {
        console.error(`Invalid role detected: ${role}`);
        await ctx.reply('Invalid role detected. Please try again.');
        return;
    }
...
async function checkForInactiveUsers(ctx) {
    ...
    try {
        ...
        if (transaction.status === 'success') {
            ...
            const options = {
                reply_markup: {
                    keyboard: [
                        [{ text: 'TOP 10 TRENDING INFLUENCERS' }],
                        [{ text: 'RENEW PLAN' }],
...
async function startSubscriptionTimer(ctx, plan, userId) {
    try {
        ...
        await bot.User.set(userId.toString(), updatedUserData);
        ...
        const checkReminders = async () => {
            ...
            for (const [daysBefore, reminderTime] of Object.entries(reminderTimes)) {
                if (reminderTime <= now && reminderTime + HOUR_IN_MS > now) {  
                    ...
                    await bot.User.set(userId.toString(), { ...userData, reminderTimes });
                }
            }
